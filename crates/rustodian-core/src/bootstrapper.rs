...
impl<'a> ProjectBootstrapper<'a> {
        ...
        for lang_det in &project.languages {
            match lang_det.language {
                ...
                Language::Go => {
                    ...
                    env.insert("GOPATH".to_string(), local_gopath.to_string_lossy().to_string());
    ...
    fn bootstrap_rust(&self, project: &Project, env: &HashMap<String, String>) -> Result<(), CoreError> {
        // Setup/Bootstrap
    ...
    fn bootstrap_node(&self, project: &Project, env: &HashMap<String, String>) -> Result<(), CoreError> {
    ...
    fn bootstrap_go(&self, project: &Project, env: &HashMap<String, String>) -> Result<(), CoreError> {
        // Setup/Bootstrap
    ...
    fn bootstrap_python(&self, project: &Project, env: &HashMap<String, String>) -> Result<(), CoreError> {
        ...
        for cmd in &["python3 -m venv .venv", "python -m venv .venv"] {
            if self.custodian.run_and_log_command(
                project,
                "bootstrap:python_venv",
                cmd,
                true,
                env.clone(),
            ).is_ok() {
        ...
        if !venv_success {
            return Err(CoreError::Internal("failed to create Python virtual environment (.venv)".to_string()));
        }
...
mod tests {
    ...
    use crate::traits::{CommandRunner, RunningProcess, ProjectStore, GitInspector, ProjectScanner};
    use crate::runner::CommandSpec;
    use crate::Custodian;
    use rustodian_types::{LanguageDetection, DetectionConfidence, Language, Project, ProjectId};
    ...
    impl RunningProcess for MockRunningProcess {
        fn id(&self) -> u32 { 1234 }
        fn wait(&mut self) -> Result<Option<i32>, CoreError> { Ok(self.exit_code) }
        fn try_wait(&mut self) -> Result<Option<Option<i32>>, CoreError> { Ok(Some(self.exit_code)) }
        fn kill(&mut self) -> Result<(), CoreError> { Ok(()) }
    ...
    impl ProjectStore for MockStore {
        fn save_project(&self, _project: &Project) -> Result<ProjectId, CoreError> { Ok(ProjectId::new()) }
        fn get_project(&self, _id: &ProjectId) -> Result<Option<Project>, CoreError> { Ok(None) }
        fn list_projects(&self) -> Result<Vec<Project>, CoreError> { Ok(vec![]) }
        fn delete_project(&self, _id: &ProjectId) -> Result<bool, CoreError> { Ok(true) }
        fn find_by_path(&self, _path: &Path) -> Result<Option<Project>, CoreError> { Ok(None) }
        fn save_scan(&self, _scan: &rustodian_types::ScanRecord) -> Result<rustodian_types::ScanId, CoreError> { Ok(rustodian_types::ScanId::new()) }
        fn get_latest_scan(&self) -> Result<Option<rustodian_types::ScanRecord>, CoreError> { Ok(None) }
        fn save_log(&self, _log: &rustodian_types::ProjectLog) -> Result<(), CoreError> { Ok(()) }
        fn list_logs(&self, _project_id: &str, _limit: usize) -> Result<Vec<rustodian_types::ProjectLog>, CoreError> { Ok(vec![]) }
        fn get_log(&self, _id: &str) -> Result<Option<rustodian_types::ProjectLog>, CoreError> { Ok(None) }
        fn get_latest_log(&self, _project_id: &str) -> Result<Option<rustodian_types::ProjectLog>, CoreError> { Ok(None) }
    }

    struct MockScanner;
    impl ProjectScanner for MockScanner {
        fn scan(&self, _root: &Path, _config: &rustodian_types::ScanConfig) -> Result<Vec<crate::traits::DiscoveredProject>, CoreError> {
            Ok(vec![])
        }
    }
    ...
    fn test_bootstrap_rust_project() {
        ...
        let runner = MockCommandRunner { commands_run: commands_run.clone() };
        let store = MockStore;
        let scanner = MockScanner;
        let git = MockGit;
        let custodian = Custodian::new(Box::new(store), Box::new(scanner), Box::new(git), Box::new(runner));
