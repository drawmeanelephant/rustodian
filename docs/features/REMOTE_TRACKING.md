...
**⚠️ NOT YET IMPLEMENTED IN DESKTOP**
While backend fetching logic in `rustodian-remote` is fully functional and tested, the Pull Requests tab in `rustodian-desktop` is currently a placeholder. It is not wired up to display fetched PR data.
...
$ cargo run --bin rustodian -- remote refresh --dest ./my_remotes
